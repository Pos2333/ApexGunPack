...
function ADS_states.aiming.update(this, context)
    ...
    if (context:getAimingProgress() < this.ADS_states.aiming_progress or not context:isStopped(context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK))) then
        context:trigger(this.INPUT_AIM_NORMAL)
    else
        this.ADS_states.aiming_progress = context:getAimingProgress()
    end
end
...
function ADS_states.aiming.transition(this, context, input)
    local track = context:getTrack(STATIC_TRACK_LINE, ADS_TRACK)
    if (input == this.INPUT_AIM_NORMAL) then
...
function main_track_states.idle.transition(this, context, input)
    ...
    if (input == INPUT_INSPECT) then
        main_track_states.stoprunning = 1
...
function main_track_states.inspect.transition(this, context, input)
    ...
    if (input == INPUT_SHOOT or context:getAimingProgress() > 0) then -- 特殊地，射击也应当打断检视
...
function movement_track_states.slide.entry(this, context)
    context:runAnimation("slide", context:getTrack(BLENDING_TRACK_LINE, MOVEMENT_TRACK), true, PLAY_ONCE_HOLD, 0.5)
end

function movement_track_states.slide.update(this, context)
    if (context:isStopped(context:getTrack(STATIC_TRACK_LINE, BASE_TRACK)) or context:isHolding(context:getTrack(BLENDING_TRACK_LINE, MOVEMENT_TRACK))) then
        context:runAnimation("slide_idle", context:getTrack(BLENDING_TRACK_LINE, MOVEMENT_TRACK), true, LOOP, 0.4)
    end
end

function movement_track_states.slide.transition(this, context, input)
...
function movement_track_states.slide.exit(this, context)
    context:runAnimation("slide_back", context:getTrack(BLENDING_TRACK_LINE, MOVEMENT_TRACK), true, PLAY_ONCE_HOLD, 0.2)
end
...
function movement_track_states.idle.transition(this, context, input)
    if (context:isCrouching() and context:isStopped(context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)) and not context:isAiming()) then
        return this.movement_track_states.slide
    end
    if (input == INPUT_RUN) then
        if (context:isStopped(context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)) or main_track_states.stoprunning == -1) then
...
function movement_track_states.run.transition(this, context, input)
    ...
    if (input == INPUT_IDLE or (main_track_states.stoprunning == 1 and not context:isStopped(context:getTrack(STATIC_TRACK_LINE, MAIN_TRACK)))) then
        return this.movement_track_states.idle
    elseif (input == INPUT_WALK) then
        return this.movement_track_states.walk
    end
...
function movement_track_states.walk.transition(this, context, input)
    ...
    if (input == INPUT_IDLE) then
        return this.movement_track_states.idle
    elseif (input == INPUT_RUN and main_track_states.stoprunning == -1) then
        return this.movement_track_states.run
    end
end
